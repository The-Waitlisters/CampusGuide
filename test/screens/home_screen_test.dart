...
class _FakeAuthService extends AuthService {
  _FakeAuthService()
    : super(auth: MockFirebaseAuth(), profileService: _MockUserProfileSvc());
...
class FakeGeocodingSuccess extends GeocodingPlatform
    with MockPlatformInterfaceMixin {
  @override
...
class FakeGeocodingThrow extends GeocodingPlatform
    with MockPlatformInterfaceMixin {
  @override
  Future<List<Placemark>> placemarkFromCoordinates(
    double latitude,
...
IndoorMap _rtTinyIndoorMap(CampusBuilding b, String roomId) {
  ...
  final floor = Floor(
    ...
    label: '1',
    rooms: [
      Room(id: roomId, name: roomId, boundary: _rtFakeRoomBoundary(0.5, 0.5)),
    ],
    navGraph: g,
  );
...
IndoorMap _rtTwoFloorIndoorMap(CampusBuilding b, String roomId) {
  final n1 = NavNode(id: roomId, type: 'room', x: 0.5, y: 0.5, name: roomId);
  ...
  final f2 = Floor(
    level: 2,
    label: '2',
    rooms: [Room(id: id2, name: id2, boundary: _rtFakeRoomBoundary(0.5, 0.5))],
    navGraph: g2,
  );
...
Future<void> main() async {
  ...
  group('findBuildingAtPoint', () {
    ...
    test(
      ...
      () {
        final result = findBuildingAtPoint(const LatLng(1, 1), [
          sgwBuilding,
          loyolaBuilding,
        ], Campus.sgw);
    ...
    test('returns null when point is outside all buildings', () {
      final result = findBuildingAtPoint(const LatLng(5, 5), [
        sgwBuilding,
        loyolaBuilding,
      ], Campus.sgw);
      expect(result, isNull);
    ...
    test('returns null for empty buildings list', () {
      final result = findBuildingAtPoint(const LatLng(1, 1), [], Campus.sgw);
      expect(result, isNull);
    });
  });

  test('defaultMarkerImageLoader loads and converts an asset image', () async {
    final Uint8List bytes = await defaultMarkerImageLoader(
      'assets/coffee.png',
      100,
    );
  ...
  group('HomeScreen', () {
    ...
    setUp(() {
      ...
      when(
        mockDataParser.getBuildingInfoFromJSON(),
    ...
    testWidgets('shows CampusToggle with SGW and Loyola', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        wrap(
    ...
    testWidgets(
      'calls getBuildingInfoFromJSON on init when parser is provided',
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        GeolocatorPlatform.instance = PositionStreamGeolocatorPlatform(
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        state.handleMapTap(
          const LatLng(1, 1),
          tester.element(find.byType(CampusToggle)),
        );
    ...
    testWidgets('search matches POI by description', (
      WidgetTester tester,
    ) async {
      final cafe = Poi(
        id: 'c1',
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        Future<IndoorMap?> testLoader(CampusBuilding b) async {
          return _rtTwoFloorIndoorMap(b, b.id == 'b1' ? 'ROOM_A' : 'ROOM_B');
        }
    ...
    testWidgets('triggerPolygonOnTap runs Polygon.onTap closure', (
      ...
      state.lastTap = const LatLng(
        1,
        1,
      ); // so _updateOnTap doesn't early return
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await tester.tap(
          find.widgetWithText(ElevatedButton, 'Set as Start').last,
        );
        await tester.pump();
        await tester.pumpAndSettle();
    ...
    testWidgets('sheet Set as Destination closes sheet', (tester) async {
      ...
      await tester.tap(
        find.widgetWithText(ElevatedButton, 'Set as Destination').last,
      );
      await tester.pump();
      await tester.pumpAndSettle();
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        GeolocatorPlatform.instance =
            TimeoutCurrentPositionGeolocatorPlatform();
    ...
    testWidgets('GPS building stays blue while another polygon is selected', (
      ...
      when(
        mockDataParser.getBuildingInfoFromJSON(),
      ).thenAnswer((_) async => [gpsB, targetB]);
    ...
    testWidgets('non-selected GPS building keeps GPS highlight colors', (
      ...
      when(
        mockDataParser.buildingsPresent,
    ...
    testWidgets('_zoomToRoute animates camera when map controller exists', (
      WidgetTester tester,
    ) async {
      final fakeMapController = FakeGoogleMapController();
    ...
    testWidgets('schedule overlay shows when toggled by test hook', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        wrap(
    ...
    testWidgets('floating UseAsStart card appears for GPS building', (
      WidgetTester tester,
    ) async {
      final building = buildTestBuilding(
        id: 'gps1',
    ...
    testWidgets('schedule overlay closes from callback path', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        wrap(
    ...
    testWidgets('schedule overlay selected-room callback path hides overlay', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        wrap(
      ...
      final ScheduleOverlay overlay = tester.widget<ScheduleOverlay>(
        find.byType(ScheduleOverlay),
      );
    ...
    testWidgets('opens schedule overlay when menu selected', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(home: HomeScreen(role: UserRole.user)),
      );
    ...
    testWidgets('guest selecting schedule shows authenticated-only snackbar', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(
    ...
    testWidgets('recenter button appears after map move and is tappable', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(
        wrap(
...
''';
    ...
    testWidgets('loadNearbyPoisForTest covers all 6 category branches', (
      ...
      final mockHttpClient = MockClient(
        (_) async => http.Response(fakeOnePlaceJson, 200),
      );
    ...
    testWidgets(
      'loadNearbyPoisForTest: restaurant only covers _finishLoadingPois',
      (WidgetTester tester) async {
        final mockHttpClient = MockClient(
          (_) async => http.Response(fakeOnePlaceJson, 200),
        );
...
''';
      final mockHttpClient = MockClient(
        (_) async => http.Response(noPhotosJson, 200),
      );
      final fakeController = FakeGoogleMapController();
    ...
    testWidgets(
      'loadNearbyPoisForTest: applyFilters loads restaurants via mock HTTP client and adds markers',
      (WidgetTester tester) async {
        final mockHttpClient = MockClient(
          (_) async => http.Response(fakeOnePlaceJson, 200),
        );
    ...
    // -------------------------------------------------------------------------
    testWidgets(
      'finishLoadingPoisForTest: covers photos loop and buildPhotoUrl',
      (WidgetTester tester) async {
        await tester.pumpWidget(
          wrap(
            home_screen.HomeScreen(
    ...
    testWidgets('simulatePoiAsStart sets startPoi state', (
      ...
      final fakeDirections = DirectionsController(
        client: FakeDirectionsClient.success(
          const RouteResult(legs: [], durationText: '0', distanceText: '0 km'),
        ),
      );
      await tester.pumpWidget(
        wrap(
    ...
    // -------------------------------------------------------------------------
    testWidgets(
      'simulatePoiAsDestination with prior startPoi covers poi route path',
      (WidgetTester tester) async {
        final startPoi = testPoi(
          id: 'ps2',
          name: 'Start',
          boundary: const LatLng(45.4, -73.5),
        );
    ...
    // -------------------------------------------------------------------------
    testWidgets(
      'simulateShowPoiDetailSheet shows BuildingDetailSheet for POI',
      (WidgetTester tester) async {
        final poi = testPoi(
          id: 'poi_sheet',
          name: 'Sheet Poi',
    ...
    testWidgets('PoiOptionMenu sort dropdown updates type', (
      WidgetTester tester,
    ) async {
      await tester.binding.setSurfaceSize(const Size(800, 1200));
      addTearDown(() async => tester.binding.setSurfaceSize(null));
    ...
    // -------------------------------------------------------------------------
    testWidgets(
      'selecting a Poi from search results calls showPoiDetailSheet',
      (WidgetTester tester) async {
        await tester.binding.setSurfaceSize(const Size(800, 1200));
        addTearDown(() async => tester.binding.setSurfaceSize(null));
    ...
    testWidgets(
      'onRoomSelected with a matching building navigates to IndoorMapScreen',
      (WidgetTester tester) async {
        // Build a building whose name matches the buildingCode we'll pass.
        final hall = buildTestBuilding(
          id: 'h1',
          name: 'HALL',
          fullName: 'Hall Building',
        );
        ...
        when(
          mockDataParser.getBuildingInfoFromJSON(),
        ).thenAnswer((_) async => [hall]);
        when(mockDataParser.buildingsPresent).thenReturn([hall]);
        ...
        // Retrieve the widget and invoke its onRoomSelected with a code that
        // matches hall.name  →  destination != null  →  navigates.
        final ScheduleOverlay overlay = tester.widget<ScheduleOverlay>(
          find.byType(ScheduleOverlay),
        );
        ...
        overlay.onRoomSelected(
          CourseScheduleEntry(
            ...
            timeText: '10:00',
            campus: 'SGW',
            buildingCode: 'HALL', // matches hall.name (case-insensitive)
          ),
        );

        await tester.pumpAndSettle();

        // The overlay should be dismissed and IndoorMapScreen pushed.
        expect(find.byType(ScheduleOverlay), findsNothing);
        expect(find.byType(IndoorMapScreen), findsOneWidget);
      },
    );

    testWidgets('simulatePoi(isStart: true) delegates to _handlePoiAsStart', (
      WidgetTester tester,
    ) async {
      final poi = testPoi(
        id: 'sp1',
      ...
      // isStart: true → calls _handlePoiAsStart
      state.simulatePoi(poi, isStart: true);
      await tester.pumpAndSettle();

      // No end point set, so the directions card is visible but shows no route.
      expect(find.byType(home_screen.HomeScreen), findsOneWidget);
    });

    testWidgets(
      'simulatePoi(isStart: false) delegates to _handlePoiAsDestination',
      (WidgetTester tester) async {
        final poi = testPoi(
          id: 'dp1',
          name: 'Dest Poi',
        ...
        // isStart: false (default) → calls _handlePoiAsDestination
    ...
    testWidgets(
      'simulateRoomSelected with no matching building hides overlay only',
      (WidgetTester tester) async {
        // buildingsPresent will be empty, so destination == null.
        await tester.pumpWidget(
          wrap(
            home_screen.HomeScreen(
              dataParser: mockDataParser,
        ...
        state.simulateRoomSelected(
          CourseScheduleEntry(
            ...
            timeText: '',
            campus: '',
            buildingCode: 'NOMATCH',
          ),
        );
        await tester.pumpAndSettle();

        // Overlay hidden; no navigation to IndoorMapScreen.
        expect(find.byType(ScheduleOverlay), findsNothing);
    ...
    testWidgets(
      'simulateRoomSelected with a matching building navigates to IndoorMapScreen',
          (WidgetTester tester) async {
        final mb = buildTestBuilding(
          id: 'mb1',
          name: 'MB',
          fullName: 'Molson Building',
        );

        when(mockDataParser.getBuildingInfoFromJSON())
            .thenAnswer((_) async => [mb]);
        when(mockDataParser.buildingsPresent).thenReturn([mb]);
        ...
        // buildingCode 'MB' matches mb.name -> destination != null
        state.simulateRoomSelected(
          CourseScheduleEntry(
            room: 'MB-1.210',
            courseCode: 'COMM 210',
        ...
        // IndoorMapScreen has an unsettling timer — fixed pump, not pumpAndSettle.
        await tester.pump();
        await tester.pump(const Duration(milliseconds: 300));
    ...
    testWidgets(
      'setLastKnownPositionForTest stores position and recenter button appears',
      (WidgetTester tester) async {
        await tester.pumpWidget(
          wrap(
            home_screen.HomeScreen(
        ...
        // Store a known position so the recenter button can appear.
        state.setLastKnownPositionForTest(const LatLng(45.5, -73.6));
        // Also trigger a camera move so _mapMoved = true.
        state.simulateCameraMove(
          const CameraPosition(target: LatLng(45.5, -73.6), zoom: 15),
        );
        await tester.pump();
    ...
    testWidgets(
      'simulatePoiSelection shows BuildingDetailSheet for the given POI',
          (WidgetTester tester) async {
        final poi = testPoi(
          id: 'sel1',
          name: 'Selection Poi',
        ...
        state.simulatePoiSelection(poi);

        // Trigger a real rebuild frame so the addPostFrameCallback registered
        // inside _showPoiDetailSheet fires on the next pump().
        // setShowResultsForTest calls setState internally, marking the widget
        // dirty — same mechanism as the setState(() {}) in simulateShowPoiDetailSheet.
        state.setShowResultsForTest(false);

        await tester.pump();         // rebuild frame fires post-frame callback -> showBottomSheet
        await tester.pumpAndSettle(); // sheet slide-in animation settles
